...
function renderDatasets() {
  $("dataBadge").textContent = `${dataStore.source.totalCatalogRows}개 중 ${dataStore.source.selectedRows}개 핵심 데이터`;
  ...
  for (let run = 0; run < runs; run += 1) {
    for (let i = 0; i < agents; i += 1) {
      ...
      for (let m = 1; m <= 12; m += 1) {
        const pressure = profile.constraints.includes("빠른 취업 선호") ? 0.025 : 0;
        const dropoutProb = clamp(0.035 + routeDropPenalty + m * 0.002 - counseling * 0.3, 0.01, 0.22);
